import { getData, sendMessage } from "./helpers";
...
export async function onNewWallPostComment({
  ...
  const [{ first_name, last_name }, { screen_name }] = [
    user.response.find(el => el.id === from_id),
    group.response.find(el => el.id === group_id),
  ];
...
export async function onNewMessage({ user_id, group_id, body }) {
  ...
  const [{ first_name, last_name }, { name, screen_name }] = [
    user.response.find(el => el.id === user_id),
    group.response.find(el => el.id === group_id),
  ];
